...
class Gui:
    ...
    def drawGrid(self,grid_class):
        grid = grid_class.getGrid()
        grid_global = grid_class.getGrid()
        blockSize = 20
        for i in range(len(grid)):
            for j in range(len(grid[i])):
                rect = pygame.Rect(j * 20, i * 20, blockSize, blockSize)
                if grid[i][j] == 1:
        ...
        for i in range(len(grid)):
            for j in range(len(grid[i])):
                state = grid[i][j]
                neighbours = self.countneighbours(grid,i,j)
        ...
        grid_class.updateValues(grid_global)
        pygame.time.wait(self.time)


    def countneighbours(self,grid, x, y):
        sum = 0
        for i in range(-1,+2):
            for j in range(-1,+2):
